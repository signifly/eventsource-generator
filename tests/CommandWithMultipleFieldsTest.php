...
class CommandWithMultipleFieldsTest extends SimpleTestCase
{
    protected $command;
    ...
    protected function setUp(): void
    {
        ...
        $this->command = new ReflectionClass('SimpleTest\CommandWithMultipleFields');
    }
    ...
    /** @test */
    public function has_a_simple_int_field_attribute()
    {
        /** @var \ReflectionProperty $method */
        $method = $this->command->getProperty('simpleInt');
        $this->assertNotNull($method);
        $this->assertFalse($method->isPublic());
        $this->assertFalse($method->isStatic());
        $this->assertEquals('int', $method->getType()->getName());
    }

    /** @test */
    public function has_a_simple_int_field_accessor()
    {
        /** @var \ReflectionMethod $method */
        $method = $this->command->getDefinedMethods()->firstWhere('name', 'simpleInt');
        $this->assertNotNull($method);
        $this->assertTrue($method->isPublic());
        $this->assertFalse($method->isStatic());
    ...
    /** @test */
    public function requires_two_arguments_in_constructor()
    {
        /** @var \ReflectionMethod $method */
        ...
        $this->assertEquals('int', $method->getParameters()[1]->getType()->getName());
    }
    ...
    /** @test */
    public function has_a_with_test_helper_method_for_int_field()
    {
        /** @var \ReflectionMethod $method */
        $method = $this->command->getDefinedMethods()->firstWhere('name', 'withSimpleInt');
        $this->assertNotNull($method);
        $this->assertEquals('self', $method->getReturnType()->getName());
        $this->assertEquals(1, $method->getNumberOfParameters());
        $this->assertEquals(1, $method->getNumberOfRequiredParameters());
